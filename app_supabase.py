...
def calculate_required():
    ...
    response = supabase.table('grades').select('score, date').eq('student_id', student_id).eq('subject_id', subject_id).execute()
    scores = []
    ...
    if current_count == 0:
        return jsonify({'current_avg': 0, 'count': 0, 'has_estimates': False, 'recommendation': '📝 Пока нет оценок в этой четверти. Добавьте первую оценку!'})

    current_avg = current_sum / current_count
    if current_avg >= target_threshold:
        return jsonify({'current_avg': round(current_avg, 2), 'count': current_count, 'has_estimates': True, 'recommendation': f'🎉 Уже достигнут порог {target_threshold}! Текущий средний: {current_avg:.2f}'})
    ...
    if not all_combinations:
        return jsonify({'current_avg': round(current_avg, 2), 'count': current_count, 'has_estimates': True, 'recommendation': f'💪 Даже при всех пятёрках невозможно достичь порога {target_threshold} в этой четверти.'})
...
def get_stats():
    ...
    response = supabase.table('grades').select('score, date').eq('student_id', student_id).eq('subject_id', subject_id).order('date', desc=True).execute()
    ...
    if scores:
        ...
        formatted_dates = [f"{r['date'].split('-')[2]}.{r['date'].split('-')[1]}.{r['date'].split('-')[0]}" for r in response.data]
...
def admin_panel():
    ...
    students_response = supabase.table('students').select('student_id, last_name').order('last_name').execute()
    subjects_response = supabase.table('subjects').select('subject_id, title').order('subject_id').execute()
    grades_response = supabase.table('grades').select('grade_id, student_id, subject_id, date, score, students(last_name), subjects(title)').order('date', desc=True).execute()
    ...
    for g in grades_response.data:
        ...
        all_grades.append({'grade_id': g['grade_id'], 'last_name': g['students']['last_name'] if g['students'] else '', 'title': g['subjects']['title'] if g['subjects'] else '', 'date': d, 'score': g['score'], 'student_id': g['student_id']})
    ...
    for student in students_response.data:
        ...
        if scores:
            stats.append({'student_id': student['student_id'], 'last_name': student['last_name'], 'count': len(scores), 'average': round(sum(scores)/len(scores), 2)})
        else:
